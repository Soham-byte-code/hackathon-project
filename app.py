...
st.caption("Smarter sourcing, fresher produce, lower carbon footprint")
...
quantity_needed = st.number_input("📦 Quantity Needed (kg)", min_value=1, step=1)
...
if st.button("🚀 Get AI Decision"):
    ...
    if matched.empty:
        ...
        total_cost = quantity_needed * best['price_per_unit']
        ...
        **Local Price (1 kg):** ₹{best['price_per_unit']}  
        **Requested Qty:** {quantity_needed} kg  
        ...
        **Decision Time:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  
        ...
        if st.button("🛒 Place Order"):
            st.success(f"🎉 Your order for {quantity_needed} kg of {best['commodity']} has been placed successfully with {best['supplier_name']}!")
            st.markdown("---")
            st.markdown("📍 Order will be delivered to your inventory from the selected supplier with optimized route and vehicle.")
