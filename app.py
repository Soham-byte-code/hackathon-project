...
if st.button("🚀 Get AI Decision"):
    ...
    if matched.empty:
        st.error("❌ No suppliers found for this commodity.")
    else:
        ...
        if best['emissions_kg'] > 10:
            ...
            if low_emission['emissions_kg'] < best['emissions_kg'] * 0.8:
                best = low_emission
                st.info("♻️ Switched to lower CO₂ emission supplier.")
        ...
        **Decision Time:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  
        ...
        if st.button("🛒 Place Order"):
            ...
            st.markdown(f"""
                ...
                🥕 You have ordered **{qty_needed} kg of {best['commodity']}**  
                📦 From supplier: **{best['supplier_name']}** (ID: {best['supplier_id']})  
                🛣️ Route: **{route}**  
                💵 **Total Cost: ₹{total_cost}**  
                ⏱️ **Estimated Time: {travel_time} hours**  
                🎉 Thank you for choosing **Walmart FreshRoute AI**!
            """)
